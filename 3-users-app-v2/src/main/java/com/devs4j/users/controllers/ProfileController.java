...
/**
 * Controller to manage Profiles of a User, it uses the service
 * {@link ProfileService}.
 *
 * @author jroldan
 * @version 1.0
...
public class ProfileController {
	...
	public ResponseEntity<ProfileEntity> getProfileById(@PathVariable("userId") Integer userId,
			@PathVariable("profileId") Integer profileId) {
		return new ResponseEntity<>(this.profileService.getByProfileIdAndUserId(userId, profileId), HttpStatus.OK);
	}

	@PostMapping
	public ResponseEntity<ProfileEntity> createProfile(@PathVariable("userId") Integer userId,
			@RequestBody final ProfileEntity profile) {
